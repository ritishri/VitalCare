...
function AddDoctor() {
  ...
  const onSubmitHandler = async(event)=>{
    ...
    try {
      ...
      //API call to backend to save the details to the database

      const { data } = await axios.post(backendUrl + '/api/admin/add-doctor', formData, {
        headers: { Authorization: `Bearer ${aToken}` }  // Send token as Bearer
      });
  ...
  return (
    <form onSubmit={onSubmitHandler} className='m-5 w-full'>
      ...
      <textarea onChange={(e)=>setAbout(e.target.value)} value={about}  className='w-full px-4 pt-2 border rounded' rows={5} placeholder='Type Here' required />
      </div>
